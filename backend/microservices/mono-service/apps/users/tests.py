...
class UserModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user1 = User.objects.create(
            id="e5e28a48-8080-11ee-b962-0242ac120002",
            username="testuser1",
        ...
        cls.rating1 = UserSongRating.objects.create(
            user=UserModelTest.user1, song=cls.song2, rating=2
        )
        cls.rating2 = UserSongRating.objects.create(
            user=UserModelTest.user1, song=cls.song1, rating=4
        )
    ...
    def setUp(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Bearer e5e28a48-8080-11ee-b962-0242ac120002"
        )
    ...
    def test_user_field_labels(self):
        ...
        # Test field labels
        self.assertEqual(
            user._meta.get_field("username").verbose_name, "username"
        )  # Adapt field names and expected labels
        self.assertEqual(
    ...
    def test_accept_friend_request(self):
        ...
        req = FriendRequest.objects.get(
            sender=UserModelTest.user1, receiver=UserModelTest.user2
        )
    ...
    def test_reject_friend_request(self):
        ...
        self.assertNotIn(UserModelTest.user2, UserModelTest.user1.friends.all())
        self.assertNotIn(UserModelTest.user1, UserModelTest.user2.friends.all())

    def test_add_user_preferences(self):
        pref = UserPreferences.objects.create(
            user=UserModelTest.user1,
    ...
    def test_create_friend_group(self):
        group = FriendGroup.objects.create(
            name="Test Group", created_by=UserModelTest.user1
        )
    ...
    def test_get_recent_addition_by_count(self):
        ...
        today_count = next(
            (item for item in song_counts if item["date"] == today_str), None
        )
    ...
    def test_get_all_data_sharing_friends(self):
        ...
        UserPreferences.objects.create(
            user=user3, data_processing_consent=True, data_sharing_consent=True
        )
        UserPreferences.objects.create(
            user=user4, data_processing_consent=True, data_sharing_consent=False
        )
        UserPreferences.objects.create(
            user=UserModelTest.user1,
            data_processing_consent=True,
        ...
        self.assertEqual(
            len(friends), 1
        )  # Only one friend (user3) gave data sharing consent
        self.assertIn(
            {"id": user3.id, "name": user3.username, "img_url": user3.img_url}, friends
        )
    ...
    def test_get_friends_favorite_genres(self):
        ...
        UserPreferences.objects.create(
            user=UserModelTest.user1, data_sharing_consent=True
        )
...
class UserSongRatingModelTest(TestCase):
    ...
    def test_user_song_rating_creation(self):
        rating = UserSongRating.objects.create(
            user=UserSongRatingModelTest.user1,
    ...
    def test_update_user_song_rating(self):
        ...
        rating.rating = 3.5
        rating.save()
        rating = UserSongRating.objects.get(
            user=UserSongRatingModelTest.user1, song=UserSongRatingModelTest.song1
        )
        # Test creation and attributes of UserSongRating
        self.assertEqual(rating.user, UserSongRatingModelTest.user1)
        self.assertEqual(rating.song, UserSongRatingModelTest.song1)
    ...
    def test_delete_user_song_rating(self):
        ...
        self.assertFalse(
            UserSongRating.objects.filter(
                user=UserSongRatingModelTest.user1, song=UserSongRatingModelTest.song1
            ).exists()
        )

    def test_average_song_rating(self):
        rating1 = UserSongRating.objects.create(
            user=UserSongRatingModelTest.user1,
            song=UserSongRatingModelTest.song1,
            rating=4.5,
        )
        rating2 = UserSongRating.objects.create(
            user=UserSongRatingModelTest.user2,
            song=UserSongRatingModelTest.song1,
            rating=3.5,
        )
        rating3 = UserSongRating.objects.create(
            user=UserSongRatingModelTest.user1,
            song=UserSongRatingModelTest.song2,
            rating=2.5,
        )
        rating4 = UserSongRating.objects.create(
            user=UserSongRatingModelTest.user2,
            song=UserSongRatingModelTest.song2,
            rating=1.5,
        )
    ...
    def test_add_song_rating(self):
        ...
        self.assertEqual(
            response.json().get("message"), "User rating added successfully"
        )
    ...
    def test_edit_song_rating(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response.json().get("message"), "User rating updated successfully"
        )
    ...
    def test_delete_song_rating(self):
        ...
        self.assertFalse(
            UserSongRating.objects.filter(user=self.user1, song=self.song1).exists()
        )
...
class UserRecommendationTest(TestCase):
    ...
    def test_recommend_you_might_like(self):
        rating1 = UserSongRating.objects.create(
            user=UserRecommendationTest.user1,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            data.get("message"), "Recommendation based on track is successful"
        )
    ...
    def test_recommend_since_you_like(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            data.get("message"), "Recommendation based on what you listen is successful"
        )
        self.assertIn("tracks_info", data)
    ...
    def test_recommend_friend_mix(self):
        ...
        rating1 = UserSongRating.objects.create(
            user=friend1, song=UserRecommendationTest.song1, rating=5
        )
        rating2 = UserSongRating.objects.create(
            user=friend1, song=UserRecommendationTest.song2, rating=5
        )
        rating3 = UserSongRating.objects.create(
            user=friend2, song=UserRecommendationTest.song3, rating=5
        )
        ...
        UserPreferences.objects.create(
            user=friend1, data_processing_consent=True, data_sharing_consent=True
        )
        UserPreferences.objects.create(
            user=friend2, data_processing_consent=True, data_sharing_consent=True
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            data.get("message"), "Recommendation based on friends is successful"
        )
        self.assertIn("tracks_info", data)
...
class CreateUserViewTest(TestCase):
    ...
    def test_create_user_no_email(self):
        ...
        response = self.client.post(
            reverse("create-user"), {}, content_type="application/json"
        )
    ...
    def test_create_user_existing_email(self, mock_create, mock_filter):
        ...
        response = self.client.post(
            reverse("create-user"), data=data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class SuggestionNotificationModelTest(TestCase):
    ...
    def test_suggest_song(self):
        ...
        data = {"receiver_user": self.user2.username, "song_id": self.song1.id}
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json().get("message"), "Song suggestion sent successfully"
        )

    def test_get_suggestions(self):

        suggestion = SuggestionNotification.objects.create(
            receiver=SuggestionNotificationModelTest.user1,
        ...
        self.assertEqual(
            received_suggestion["suggester_name"], suggestion.suggester.username
        )
    ...
    def test_set_suggestion_seen(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json().get("message"), "Suggestions are marked as seen"
        )
        self.assertTrue(
            SuggestionNotification.objects.get(
                receiver=SuggestionNotificationModelTest.user1
            ).is_seen
        )
    ...
    def test_delete_suggestion(self):
        ...
        suggestion_id = SuggestionNotification.objects.create(
            suggester=self.user1, receiver=self.user2, song=self.song1
        ).id
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json().get("message"), "Suggestion deleted successfully"
        )
        self.assertFalse(
            SuggestionNotification.objects.filter(id=suggestion_id).exists()
        )
...
class UserPreferencesCreateTest(TestCase):
    ...
    @patch("users.views.User.objects.get")
    def test_user_preferences_user_not_exist(
        self, mock_get, mock_preferences_get_or_create
    ):
        mock_get.side_effect = User.DoesNotExist
        data = {"user": "nonexistinguser"}
        response = self.client.post(
            reverse("user-preferences-create"),
            json.dumps(data),
    ...
    @patch("users.views.UserPreferences.objects.get_or_create")
    @patch("users.views.User.objects.get")
    def test_user_preferences_create_new(
        self, mock_user_get, mock_preferences_get_or_create
    ):
        mock_user = MagicMock(spec=User)  # Create a mock user
        mock_user_get.return_value = (
            mock_user  # Return the mock user when User.objects.get is called
        )
        mock_preferences_get_or_create.return_value = (
            MagicMock(spec=UserPreferences),
            True,
        )
    ...
    @patch("users.views.UserPreferences.objects.get_or_create")
    @patch("users.views.User.objects.get")
    def test_user_preferences_update_existing(
        self, mock_user_get, mock_preferences_get_or_create
    ):
        mock_user = MagicMock(spec=User)  # Create a mock user
        mock_user_get.return_value = (
...
class UserSongsWithGenreTest(TestCase):
    ...
    def test_valid_request_with_zero_rated_songs(
        self, mock_user_song_ratings, mock_user_get
    ):
...
class UserSongsWithTempoTest(TestCase):
    ...
    def test_missing_tempo_name(self):
        ...
        response = self.client.get(
            reverse("get-songs-tempo"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class UserSongsWithArtistTest(TestCase):
    ...
    def test_missing_tempo_name(self):
        data = {"number_of_songs": "5"}
        response = self.client.get(
            reverse("get-songs-artist"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)

    @patch("users.views.User.objects.get")
    @patch("users.views.UserSongRating.objects.filter")
    def test_valid_request_with_irrelevant_artist(
        self, mock_user_song_ratings, mock_user_get
    ):
        # Set up the mocks
        ...
        mock_song_rating.song = mock_song
        mock_user.usersongrating_set.prefetch_related.return_value.all.return_value = [
            mock_song_rating
        ]
        ...
        mock_song.artistsong_set.prefetch_related.return_value.all.return_value = [
            mock_artist_song
        ]
...
class UserRecentlyAddedSongTest(TestCase):
    ...
    def test_invalid_number_of_songs_type(self):
        ...
        )  # Assert that if the number is noninteger type, we return 400 Bad Request
    ...
    def test_negative_number_of_songs(self):
        ...
        self.assertEqual(
            response.status_code, 400
        )  # Assert that if the number is negative, we return 400 Bad Request
...
class UserFavoriteSongsTest(TestCase):
    ...
    def test_multiple_songs(self):
        ...
        response = self.client.get(
            reverse("get-favorite-songs"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            len(response.json()["songs"]), 2
        )  # Assert that the number of songs returned is 2
        self.assertEqual(
            response.json()["songs"][0]["id"], "song1"
        )  # Assert that the first song is song1 since it has higher rating
        self.assertEqual(
            response.json()["songs"][1]["id"], "song2"
        )  # Assert that the second song is song2 since it has lower rating
    ...
    def test_more_songs_exist_than_wanted(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            len(response.json()["songs"]), 1
        )  # Assert that the number of songs returned is 1 since we requested 1
        self.assertEqual(
            response.json()["songs"][0]["id"], "song1"
        )  # Assert that the song having higher rating is returned which is song1
...
class UserFavoriteGenresTest(TestCase):
    ...
    def test_multiple_songs_with_different_genre(self):
        data = {"number_of_songs": "5"}
        response = self.client.get(
            reverse("get-favorite-genres"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content)
        self.assertEqual(
            response_json.get("TestGenre"), 2
        )  # Must be 2 since we have 2 songs of this genre
        self.assertEqual(
            response_json.get("TestGenre2"), 1
        )  # Must be 1 since we only have 1 song of this genre
    ...
    def test_limited_songs_with_different_genre_one_for_each(self):
        ...
        self.assertEqual(
            response_json.get("TestGenre"), 2
        )  # Must be 2 according to our setup
        self.assertEqual(
            response_json.get("TestGenre2"), None
        )  # Must be 0 since the song with genre 2 has the lowest rating
...
class UserFavoriteArtistsTest(TestCase):
    ...
    def test_multiple_songs_with_multiple_artists(self):
        data = {"number_of_songs": "5"}
        response = self.client.get(
            reverse("get-favorite-artists"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content)
        self.assertEqual(
            response_json.get("Semicenk"), 1
        )  # Must be 1 since we have 1 song of this artist
        self.assertEqual(
            response_json.get("Rast"), 1
        )  # Must be 1 since we have 1 song of this artist, even though the song has multiple artists
    ...
    def test_limited_songs_with_different_genre_one_for_each(self):
        ...
        response_json = json.loads(response.content)
        self.assertEqual(
            response_json.get("Semicenk"), None
        )  # Must be 0 since the song of Semicenk is not in the highest rated 2 songs
        self.assertEqual(
            response_json.get("Ceza"), 1
        )  # Must be 1 since Ceza has a song that is included in highest rated 2 songs
...
class GetAllRatedSongsTest(TestCase):
    ...
    def test_invalid_number_of_songs(self):
        data = {"number_of_songs": "invalid"}
        response = self.client.get(
            reverse("get-all-recent"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_get_different_users_rated_songs(self):
        ...
        response_json = json.loads(response.content)
        self.assertEqual(
            len(response_json["songs"]), 2
        )  # Must be 2 since we have 2 ratings even if they are rated by different users
        self.assertEqual(
            response_json["songs"][0]["name"], "Neyim Var Ki"
        )  # Must be this song since it has higher rating
        self.assertEqual(
            response_json["songs"][1]["name"], "Canin Sagolsun"
        )  # Must be this song since it has higher rating
...
class SendFriendRequestTest(TestCase):
    ...
    def test_wrong_method(self):
        ...
        response = self.client.get(
            reverse("send-friend-request"), data, content_type="application/json"
        )
...
class GetIncomingAndOutgoingRequestsTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        FriendRequest.objects.create(
            sender=user2, receiver=user1, status=RequestStatus.PENDING
        )
        FriendRequest.objects.create(
            sender=user3, receiver=user1, status=RequestStatus.REJECTED
        )
        FriendRequest.objects.create(
            sender=user1, receiver=user2, status=RequestStatus.PENDING
        )
        FriendRequest.objects.create(
            sender=user1, receiver=user4, status=RequestStatus.PENDING
        )
        FriendRequest.objects.create(
            sender=user1, receiver=user3, status=RequestStatus.REJECTED
        )
    ...
    def test_get_returns_only_pending_requests(self):
        response = self.client.get(
            reverse("get-all-incoming-requests"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        self.assertEqual(
            len(response.json()["requests"]), 1
        )  # To make sure that only pending requests are returned
        self.assertEqual(response_data["requests"][0]["name"], "testuser2")

    def test_get_returns_only_pending_outgoing_requests(self):
        response = self.client.get(
            reverse("get-all-outgoing-requests"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        self.assertEqual(
            len(response.json()["requests"]), 2
        )  # To make sure that only pending requests are returned
        self.assertEqual(response_data["requests"][0]["name"], "testuser2")
    ...
    def test_get_incoming_count(self):
        response = self.client.get(
            reverse("get-incoming-count"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        self.assertEqual(
...
class AcceptIncomingRequestTest(TestCase):
    ...
    def test_accept_request_success(self):
        data = json.dumps({"username": "testuser2"})
        response = self.client.post(
            reverse("accept-friend-request"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class RejectIncomingRequestTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        FriendRequest.objects.create(
            sender=user3, receiver=user1, status=RequestStatus.ACCEPTED
        )
    ...
    def test_reject_request_success(self):
        data = json.dumps({"username": "testuser2"})
        response = self.client.post(
            reverse("reject-friend-request"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class CancelOutgoingRequestTest(TestCase):
    ...
    def test_cancel_request_success(self):
        data = json.dumps({"username": "testuser2"})
        response = self.client.post(
            reverse("cancel-friend-request"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class GetAllFriendsTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        )  # user1 is friend with user2, but not with user3
    ...
    def test_get_all_friends(self):
        response = self.client.get(
            reverse("get-all-friends"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        self.assertEqual(
            len(response_data["friends"]), 1
        )  # Assert that only one friend is returned, which is user2
...
class RemoveFriendFromGroupTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        group = FriendGroup.objects.create(
            id="1", name="TestGroup", description="TestDescription", created_by=user1
        )
        group.friends.set([user1, user2])
        group2 = FriendGroup.objects.create(
            id="2", name="TestGroup2", description="TestDescription2", created_by=user2
        )
    ...
    def test_remove_friend_from_group_success(self):
        ...
        response = self.client.put(
            reverse("remove-friend-from-group"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_remove_friend_but_not_authorized(self):
        ...
        self.assertEqual(
            response_data["error"],
            "You are not authorized to remove friends from this group",
        )
...
class DeleteFriendGroupTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        group.friends.set([user1, user2])
        group2 = FriendGroup.objects.create(
            id="2", name="TestGroup", description="TestDescription", created_by=user2
        )
        group2.friends.set([user1, user2])
    ...
    def test_delete_missing_param(self):
        response = self.client.delete(
            reverse("delete-friend-group"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        response_data = json.loads(response.content)
    ...
    def test_delete_friend_group_wrong_method(self):
        data = {"group_id": "1"}
        response = self.client.post(
            reverse("delete-friend-group"), data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 405)
...
class GetAllFriendGroupsTest(TestCase):
    ...
    def test_get_all_friend_groups(self):
        response = self.client.get(
            reverse("get-all-friend-groups-of-user"),
            {},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        self.assertEqual(
            len(response_data["friend_groups"]), 2
        )  # Assert that both groups are returned no matter who created them
...
class SavePlaylistTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.artist = Artist.objects.create(
            id="artist1", name="Artist 1", bio="Artist Bio"
        )
    ...
    def test_save_playlist_invalid_json_format(self):
        ...
        response = self.client.post(
            reverse("save_playlist"), data=invalid_data, content_type="application/json"
        )
    ...
    def test_save_playlist_unexpected_error(self):
        ...
        with self.assertRaises(Exception):
            response = self.client.post(
                reverse("save_playlist", kwargs={"userid": self.user.id}),
...
class FriendGroupModelTest(TestCase):
    ...
    def test_create_friend_group(self):
        ...
        data = {"name": "Group 3", "description": "A group for new friends"}
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response.json().get("message"), "Friend group created successfully"
        )
    ...
    def test_get_friend_group_by_id(self):
        ...
        response_extended = self.client.get(
            url, {"group_id": self.group1.id, "extended": True}
        )
    ...
    def test_add_friend_to_group(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response_data["message"], "Friend was added to the group successfully"
        )
