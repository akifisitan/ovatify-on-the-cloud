...
def search_db(request, userid):
    if request.method == "GET":
        ...
        songs = (
            ...
            .filter(Q(rank__gte=0.4) | Q(similarity__gt=0.4))
            .distinct("id")
        )
        ...
        return JsonResponse(
            {"message": "Songs found", "songs_info": songs_info}, status=200
        )
...
def get_song_by_id(request, userid):
    ...
    recorded_environment_long_form = dict(RecordedEnvironment.choices)[
        ...
        if request.method != "POST":
            ...
            if track is None:
                ...
                if audio_features is None:
                    return JsonResponse(
                        {"error": "No audio features found for the song"}, status=400
                    )
                else:
                    audio_features = audio_features[0]
                    recorded_environment = (
                        RecordedEnvironment.LIVE
                        if audio_features["liveness"] >= 0.8
                        else RecordedEnvironment.STUDIO
                    )
                    tempo = (
                        Tempo.FAST
                        if audio_features["tempo"] >= 120
                            ...
                            Tempo.MEDIUM
                            if 76 <= audio_features["tempo"] < 120
                            else Tempo.SLOW
                        )
                    )
                    ...
                    new_song, created = Song.objects.get_or_create(
                        ...
                        tempo=tempo,
                        duration=str(
                            timedelta(seconds=int(track["duration_ms"] / 1000))
                        ),
                    ...
                    if created:
                        ...
                        for artist in track["artists"]:
                            ...
                            artist_img_url = (
                                artist_img if artist_img is not None else None
                            )
                            ...
                            if (
                                ...
                                or db_artist.img_url != artist_img_url
                            ):
                                db_artist.bio = artist_bio
                        ...
                        album_name = (
                            ...
                            else track["name"].title() + " - Single"
                        )
                        album_instance, album_created = Album.objects.get_or_create(
                            id=track["album"]["id"],
                            name=album_name,
                            release_year=track["album"]["release_date"][:4],
                            img_url=track["album"]["images"][0]["url"],
                    ...
                    if rating > 0 and rating <= 5:
                        try:
                            ...
                            existing_rating = UserSongRating.objects.filter(
                                user=user, song=new_song
                            )

                            if created and existing_rating.count() == 0:
                                user_song_rating, rating_created = (
                                    UserSongRating.objects.get_or_create(
                                        user=user, song=new_song, rating=rating
                                    )
                                )
                                return JsonResponse(
                                    {"message": f"Rating & song is added successfully"},
                                    status=200,
                                )
                                ...
                                return JsonResponse(
                                    {"message": f"Rating is added successfully"},
                                    status=200,
                                )
                            else:
                                return JsonResponse(
                                    {
                                        "message": f"You have already added this song, please update your rating via your library"
                                    },
                                    status=400,
                                )
                        except User.DoesNotExist:
                        ...
                        if created:
                            return JsonResponse(
                                {"message": f"Song added successfully"}, status=201
                            )
                        else:
                            return JsonResponse(
                                {"message": f"You have already added this song"},
                                status=400,
                            )
...
def search_spotify(request, userid):
    try:
        if request.method == "GET":
            ...
            return JsonResponse(
                {"message": "Search successful", "results": search_list}, status=200
            )
        else:
            return JsonResponse({"error": "Invalid method"}, status=400)
        ...
        if request.method == "GET":
            ...
            try:
                ...
                if total_ratings > 0:
                    ...
                    return JsonResponse(
                        {"error": "No ratings available for this song"}, status=404
                    )
            except UserSongRating.DoesNotExist:
                return JsonResponse(
                    {"error": "No ratings available for this song"}, status=404
...
def get_demanded_genres(request, userid):
    ...
    try:
        ...
        if number_of_genres == -1:
            ...
            genres = (
                Genre.objects.all().values().order_by("-updated_at")[:number_of_genres]
            )
        serialized_genres = [
            {
                "id": genre["id"],
...
def get_songs_by_genre(request, userid):
    ...
    try:
        ...
        if number_of_songs == -1:
            songs = (
                Song.objects.filter(genres__name=genre_name)
                .order_by("-created_at")
                .all()
            )
        else:
            songs = Song.objects.filter(genres__name=genre_name).order_by(
                "-created_at"
            )[:number_of_songs]
        serialized_songs = [
            {
...
def get_random_genres(request, userid):
    ...
    try:
        ...
        if number_of_genres <= 0:
            return JsonResponse({"error": "Invalid number of genres"}, status=400)
        genres = Genre.objects.annotate(num_songs=Count("genresong")).order_by(
            "-num_songs"
        )[:number_of_genres]
        serialized_genres = [
            {
    ...
    if request.method == "GET":
        ...
        if search_text:
            ...
            artists = (
                ...
                .filter(Q(rank__gte=0.2) | Q(similarity__gt=0.2))
                .order_by("-similarity")[:number_of_artists]
            )

        else:
            return JsonResponse({"error": "Missing search text"}, status=400)
        ...
        return JsonResponse(
            {"message": "Artists found", "artists_info": artists_info}, status=200
        )
...
def search_genres(request, userid):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"message": "Genres found", "genres_info": genres_info}, status=200
        )
...
def get_banger_songs(request, userid):
    if request.method == "GET":
        ...
        if songs.exists():
            ...
            return JsonResponse(
                {"message": "Random Banger song found", "song_info": song_info},
                status=200,
            )
        else:
            return JsonResponse(
                {"message": "No Banger songs found with the given filters"}, status=404
            )
