...
def bulk_get_or_create(
    model: Model, data: List, unique_field: str
        ...
        if audio_features is None:
            ...
            tempo = (
                ...
                if audio_features["tempo"] >= 120
                    ...
                    Tempo.MEDIUM if 76 <= audio_features["tempo"] < 120 else Tempo.SLOW
                )
            )
...
def addBatchSongFromSpotify(genres=None, artists=None, is_from_album=False):
    ...
    for artist in all_artists:
        ...
        top_tracks = []
        if is_from_album:
            albums = sp.artist_albums(artist_id=artist.id, country="TR", limit=10)
            for album in albums["items"]:
                album_tracks = sp.album_tracks(album_id=album["id"], limit=15)
        ...
        if is_from_album is False:
            top_tracks = sp.artist_top_tracks(artist_id=artist.id, country="TR")
...
def serializePlaylistInfo(playlist: Playlist):
    ...
    song_imgs = [song.img_url for song in playlist.songs.all()[:count]]
    return {
        "id": playlist.id,
        "name": playlist.name,
        ...
        "friend_group_id": (
            playlist.friend_group.id if playlist.friend_group is not None else None
        ),
    }
...
def serializeSongMinimum(song: Song):
    return {
        ...
        "main_artist": (
            ...
            if song.artists.all().first() is not None
            else None
        ),
    }
