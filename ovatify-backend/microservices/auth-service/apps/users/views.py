...
from auth_service.firebase_auth import token_required
...
def user_preferences_create(request, userid):
    try:
        ...
        try:
            ...
            return JsonResponse({"error": "user not found"}, status=404)
        user_preferences, preferences_created = UserPreferences.objects.get_or_create(
            user=user,
            data_processing_consent=data.get("data_processing_consent", True),
            data_sharing_consent=data.get("data_sharing_consent", True),
        )

        if preferences_created:
            return JsonResponse(
                {"message": "UserPreferences created successfully."}, status=201
            )
        else:
            return JsonResponse(
                {"message": "UserPreferences updated successfully."}, status=200
            )
...
def add_song_rating(request, userid):
    try:
        if request.method == "POST":
            ...
            try:
                user_rating, created_rating = UserSongRating.objects.get_or_create(
                    user=user, song=song, rating=rating
                )

                if not created_rating:
                    return JsonResponse(
                        {"error": "User rating already exists"}, status=400
                    )
                return JsonResponse(
                    {"message": "User rating added successfully"}, status=201
                )
            except IntegrityError:
                return JsonResponse(
                    {"error": "Integrity Error: Invalid user or song reference"},
                    status=404,
                )
        else:
...
def delete_song_rating(request, userid):
    try:
        if request.method == "DELETE":
            ...
            return JsonResponse(
                {"message": "User rating deleted successfully"}, status=204
            )
        else:
            return JsonResponse({"error": "Invalid method"}, status=400)
    except KeyError as e:
...
def user_songs_with_genre(request, userid):
    ...
    try:
        ...
        songs = [
            song_rating.song for song_rating in user_songs_ratings
        ]  # Get the song objects from the ratings
        matching_songs = []
        for song in songs:
        ...
        serialized_songs = [
            {
                ...
                "recorded_environment": RecordedEnvironment(
                    song.recorded_environment
                ).label,
                "replay_count": song.replay_count,
                "version": song.version,
    ...
    try:
        ...
        if number_of_songs is None:
            ...
            user_songs_ratings = user.usersongrating_set.prefetch_related(
                "song"
            ).order_by("-created_at")[:number_of_songs]
        songs = [
            song_rating.song for song_rating in user_songs_ratings
        ]  # Get the song objects from the ratings
        serialized_songs = [
...
def get_favorite_songs(request, userid):
    ...
    try:
        ...
        if number_of_songs is None:
            ...
            user_songs_ratings = user.usersongrating_set.prefetch_related(
                "song"
            ).order_by("-rating", "-updated_at")[:number_of_songs]
        songs = [
            song_rating.song for song_rating in user_songs_ratings
        ]  # Get the song objects from the ratings
...
def get_favorite_genres(request, userid):
    ...
    try:
        ...
        if number_of_songs is None:
            ...
            user_songs_ratings = user.usersongrating_set.prefetch_related(
                "song"
            ).order_by("-rating")[:number_of_songs]
...
def get_all_song_genres(request, userid):
    ...
    context = {"GenreSongRelation": list(genresong)}
    return JsonResponse(context, status=200)
...
def get_all_incoming_requests(request, userid):
    ...
    try:
        ...
        incoming_requests = FriendRequest.objects.filter(
            receiver=user, status=RequestStatus.PENDING
        ).prefetch_related("sender")
        senders = [single_request.sender for single_request in incoming_requests]
        sender_users = [
            {"id": sender.id, "name": sender.username, "img_url": sender.img_url}
...
def get_all_outgoing_requests(request, userid):
    ...
    try:
        ...
        outgoing_requests = FriendRequest.objects.filter(
            sender=user, status=RequestStatus.PENDING
        ).prefetch_related("receiver")
        receivers = [single_request.receiver for single_request in outgoing_requests]
        receiver_users = [
            {"id": receiver.id, "name": receiver.username, "img_url": receiver.img_url}
...
def send_friend_request(request, userid):
    ...
    try:
        ...
        if incoming_request and incoming_request.status == RequestStatus.PENDING:
            return JsonResponse(
                {
                    "error": f"There is already a pending request coming from user {receiverUser}"
                },
                status=409,
            )
        ...
        if existing_friend_request:
            if existing_friend_request.status == RequestStatus.PENDING:
                return JsonResponse(
                    {"error": "There is already a pending request"}, status=400
                )
            else:
                existing_friend_request.status = RequestStatus.PENDING
                existing_friend_request.save()
                return JsonResponse(
                    {"message": "Friend request sent successfully."}, status=200
                )
...
def accept_friend_request(request, userid):
    ...
    try:
        ...
        if user.friends.filter(id=requesterUser.id).exists():
            return JsonResponse({"error": "User is already a friend"}, status=400)
        pendingRequest = FriendRequest.objects.filter(
            sender=requesterUser, receiver=user
        ).first()
        if not pendingRequest:
            return JsonResponse(
                {"error": f"There is no request found from the user: {requesterName} "},
                status=404,
            )
        if pendingRequest.status == RequestStatus.PENDING:
            ...
            return JsonResponse(
                {"message": f"You are already a friend with {requesterName}"},
                status=400,
            )
        elif pendingRequest.status == RequestStatus.REJECTED:
            return JsonResponse(
                {
                    "message": "This request has been rejected before,"
                    " you cannot accept now."
                },
                status=400,
            )
    except User.DoesNotExist:
...
def reject_friend_request(request, userid):
    ...
    try:
        ...
        if pendingRequest.status == RequestStatus.PENDING:
            ...
            return JsonResponse(
                {"message": "This request has already been accepted"}, status=400
            )
        elif pendingRequest.status == RequestStatus.REJECTED:
            return JsonResponse(
                {"message": "You have already rejected this request."}, status=400
            )
    except User.DoesNotExist:
        return JsonResponse({"error": "user not found"}, status=404)
    except Exception as e:
...
def cancel_friend_request(request, userid):
    ...
    try:
        ...
        if receiverUser is None:
            return JsonResponse(
                {"error": f"There is no user found with username {receiverName}"},
                status=404,
            )
        pendingRequest = FriendRequest.objects.filter(
            sender=user, receiver=receiverUser
        ).first()
        if not pendingRequest:
            return JsonResponse(
                {"error": f"There is no request found for the user: {receiverName}"},
                status=404,
            )
        if pendingRequest.status == RequestStatus.PENDING:
            pendingRequest.hard_delete()
            return JsonResponse(
                {"message": f"You canceled your friend request to {receiverName}."},
                status=200,
            )
...
def get_all_friends(request, userid):
    ...
    try:
        ...
        all_friends = [
            {"id": friend.id, "name": friend.username, "img_url": friend.img_url}
...
def get_recent_addition_by_count(request, userid):
    ...
    try:
        ...
        user_songs_per_day = (
            user.usersongrating_set.filter(created_at__gte=start_datetime)
        ...
        formatted_song_count_by_day = {
            date.strftime("%d-%m"): count for date, count in song_count_by_day.items()
        }
        song_count_list = [
            {"date": date, "count": count}
            for date, count in formatted_song_count_by_day.items()
        ]
...
def get_profile_stats(request, userid):
    ...
    try:
        ...
        rating_aggregation = user.usersongrating_set.aggregate(
            total_ratings=Count("rating"), sum_ratings=Sum("rating")
        )
        ...
        return JsonResponse(
            {
                ...
                "rating_average": float(round(rating_average, 2)),
            },
            status=200,
        )
    except User.DoesNotExist:
...
def export_by_genre(request, userid):
    ...
    try:
        ...
        user_songs_ratings = (
            user.usersongrating_set.prefetch_related(
        ...
        export_songs = [
            rating.filtered_songs
            for rating in user_songs_ratings
            if rating.filtered_songs
        ]
        serialized_songs = [
            {
                "id": song.id,
                ...
                "instruments": [
                    instrument.name for instrument in song.instruments.all()
                ],
                "release_year": song.release_year,
        ...
        with open(tmp_file_path, "r", encoding="utf-8") as file:
            response = HttpResponse(
                file, content_type="application/json; charset=utf-8"
            )
    ...
    try:
        ...
        for song_data in data:
            ...
            try:
                ...
                return_data.append(
                    {
                        "error": f"{song_name} does not exist in database, please add it manually"
                    }
                )
                continue
            ...
            if rating > 0 and rating <= 5:
                ...
                if len(existing_rating) == 0:
                    ...
                    return_data.append(
                        {
                            "message": f"Rating of {rating} is added to {song_name} successfully"
                        }
                    )
                    continue
                else:
                    return_data.append(
                        {
                            "error": f"You have already rated {song_name}, please update your rating via your library"
                        }
                    )
                    continue
        ...
        return JsonResponse(
            {"error": f"Unexpected error: {e}", "return_data": return_data}, status=500
        )
...
def get_library_artist_names(request, userid):
    ...
    try:
        ...
        if not artists:
            return JsonResponse(
                {"error": "No artist is found for the user"}, status=404
            )
...
def get_all_data_sharing_friends(request, userid):
    ...
    try:
        ...
        all_friends = [
            {"id": friend.id, "name": friend.username, "img_url": friend.img_url}
            for friend in friends
            if friend.userpreferences.data_sharing_consent
        ]
        return JsonResponse({"friends": all_friends}, status=200)
    except Exception as e:
        return JsonResponse({"error": "Database error"}, status=500)
...
def add_song_to_playlist(request, userid):
    ...
    try:
        ...
        if song in playlist.songs.all():
            return JsonResponse(
                {"error": f"{song.name} already exists in playlist {playlist.name}"},
                status=400,
            )

        playlist.songs.add(song)
        return JsonResponse(
            {"message": f"{song.name} is added to playlist {playlist.name}"}, status=200
        )
...
def remove_song_from_playlist(request, userid):
    ...
    try:
        ...
        if song not in playlist.songs.all():
            return JsonResponse(
                {"error": f"{song.name} does not exist in playlist {playlist.name}"},
                status=400,
            )
        ...
        return JsonResponse(
            {
                "message": f"Song {song.name} removed from playlist {playlist.name} successfully"
            },
            status=200,
        )
    except Exception as e:
...
def create_empty_playlist(request, userid):
    ...
    try:
        ...
        p = Playlist.objects.create(name=name, description=description, user=user)
        return JsonResponse(
            {"message": f"Playlist {p.name} created succesfully"}, status=201
        )
    except Exception as e:
        return JsonResponse({"Unexpected error": str(e)}, status=500)
...
def edit_playlist(request, userid):
    ...
    try:
        ...
        serialized_pl = serializePlaylistInfo(playlist)
        return JsonResponse(
            {"message": "Playlist edited successfully", "playlist": serialized_pl},
            status=200,
        )
    except Exception as e:
        return JsonResponse({"Unexpected error": str(e)}, status=500)
...
def get_global_all_friend_groups(request, userid):
    ...
    try:
        ...
        context = {
            "friend_groups": [
                serializeFriendGroupSimple(friend_group)
    ...
    try:
        ...
        if group_name:
            friend_group = FriendGroup.objects.create(
                name=group_name, description=group_description, created_by=creating_user
            )
            friend_group.friends.add(creating_user)
            friend_group.save()
        else:
            return JsonResponse(
                {"error": "Please provide a name for the friend group."}, status=400
            )
        return JsonResponse(
            {
                "message": "Friend group created successfully",
                "id": f"{friend_group.id}",
            },
            status=201,
        )
    except User.DoesNotExist:
...
def get_friend_group_by_id(request, userid):
    ...
    try:
        ...
        if group_id is None:
            return JsonResponse(
                {"error": "Please provide the id for friend group."}, status=400
            )
        ...
        return JsonResponse(
            {"error": "Friend group does not exist anymore."}, status=404
        )
    except Exception as e:
        logging.error(f"error: {e}")
        return JsonResponse({"error": str(e)}, status=500)
...
def add_friend_to_group(request, userid):
    ...
    try:
            ...
            return JsonResponse(
                {
                    ...
                    "please check friend name and group id fields"
                },
                status=400,
            )
        if not isinstance(group_id, int):
            return JsonResponse(
                {"error": "Invalid group id, please check the group id field"},
                status=400,
            )
        user = User.objects.get(id=userid)
        friend_group = FriendGroup.objects.get(id=group_id)
        if friend_group.created_by != user:
            return JsonResponse(
                {"error": "You are not authorized to add friends to this group"},
                status=401,
            )
        friend = User.objects.get(username=friend_name)
        if user == friend:
            return JsonResponse(
                {"error": "You cannot add yourself to a friend group"}, status=400
            )
        if friend in friend_group.friends.all():
            return JsonResponse(
                {
                    "error": f"User {friend_name} is already a member of the friend group"
                },
                status=400,
            )
        friend_group.friends.add(friend)
        return JsonResponse(
            {"message": "Friend was added to the group successfully"}, status=200
        )
    except FriendGroup.DoesNotExist:
        return JsonResponse(
            {"error": "Friend group with the given id does not exist"}, status=404
        )
    except User.DoesNotExist:
        return JsonResponse(
            {"error": "Friend with the given username does not exist"}, status=404
        )
    except Exception as e:
        logging.error(f"error: {e}")
        return JsonResponse({"error": str(e)}, status=500)
...
def remove_friend_from_group(request, userid):
    ...
    try:
        ...
        if friend_group.created_by != user:
            return JsonResponse(
                {"error": "You are not authorized to remove friends from this group"},
                status=401,
            )
        friend = User.objects.get(username=friend_name)
        if user == friend:
            return JsonResponse(
                {"error": "You cannot remove yourself from a friend group"}, status=400
            )
        if friend not in friend_group.friends.all():
            return JsonResponse(
                {"error": f"User {friend_name} is not a member of the friend group"},
                status=400,
            )
        friend_group.friends.remove(friend)
        return JsonResponse(
            {"message": "Friend was removed from the group successfully"}, status=200
        )
    except FriendGroup.DoesNotExist:
        return JsonResponse(
            {"error": "Friend group with the given id does not exist"}, status=404
...
def delete_friend_group(request, userid):
    ...
    try:
        ...
        if group_id is None:
            return JsonResponse(
                {"error": "Please check the group id field."}, status=400
            )
        ...
        if friend_group.created_by != user:
            return JsonResponse(
                {"error": "You are not authorized to delete this friend group."},
                status=401,
            )
        friend_group.delete()
        return HttpResponse(status=204)
...
def edit_friend_group(request, userid):
    ...
    try:
        ...
        if friend_group.created_by != user:
            return JsonResponse(
                {"error": "You are not authorized to update this friend group."},
                status=401,
            )
        if description is not None:
...
def get_all_friend_groups_of_user(request, userid):
    ...
    try:
        ...
        if not extended:
            friend_groups_serialized = [
                serializeFriendGroupSimple(group) for group in friend_groups_of_user
            ]
        else:
            friend_groups_serialized = [
                serializeFriendGroupExtended(group) for group in friend_groups_of_user
            ]
...
def create_empty_playlist_in_group(request, userid):
    ...
    try:
        ...
        if name is None:
            return JsonResponse(
                {"error": "Please provide the playlist name."}, status=400
            )
        if description is None:
            description = ""
        user = User.objects.get(id=userid)
        ...
        if user not in friend_group.friends.all():
            return JsonResponse(
                {"error": "You are not authorized to create a playlist in this group"},
                status=401,
            )
        playlist = Playlist.objects.create(
            name=name, description=description, friend_group=friend_group
        )
        serialized_pl = serializePlaylist(playlist)
        return JsonResponse(
            {"message": "Playlist created successfully", "playlist": serialized_pl},
            status=201,
        )
    except User.DoesNotExist:
        return JsonResponse({"error": "User does not exist"}, status=404)
...
def delete_playlist_from_group(request, userid):
    ...
    try:
        ...
        if not playlist_id or not isinstance(playlist_id, int):
            return JsonResponse(
                {"error": "Please check the playlist id field."}, status=400
            )
        playlist = Playlist.objects.get(id=playlist_id)
        user = User.objects.get(id=userid)
        if user not in playlist.friend_group.friends.all():
            return JsonResponse(
                {"error": "You are not authorized to delete this playlist"}, status=401
            )
        playlist.delete()
        return HttpResponse(status=204)
                ...
                serialized_notifications.append(
                    {
                        ...
                        "suggester_img_url": User.objects.get(
                            username=notification.suggester.username
                        ).img_url,
    ...
    try:
        if request.method != "GET":
            ...
            try:
                ...
                return JsonResponse({"error": "User not found"}, status=404)
            notifications = SuggestionNotification.objects.filter(
...
def set_suggestion_seen(request, userid):
    try:
        if request.method != "PUT":
            ...
            return JsonResponse(
                {"message": "Suggestions are marked as seen"}, status=200
            )
    except KeyError as e:
        logging.error(f"A KeyError occurred: {str(e)}")
        return JsonResponse({"error": str(e)}, status=500)
...
def delete_suggestion(request, userid):
    try:
        if request.method != "DELETE":
            ...
            notification.delete()
            return JsonResponse(
                {"message": "Suggestion deleted successfully"}, status=200
            )
    except KeyError as e:
        logging.error(f"A KeyError occurred: {str(e)}")
        return JsonResponse({"error": str(e)}, status=500)
...
def save_playlist(request, userid):
    if request.method == "POST":
        try:
            ...
            playlist = Playlist.objects.create(
                name=data["name"], description=data["description"], user=user
            )
