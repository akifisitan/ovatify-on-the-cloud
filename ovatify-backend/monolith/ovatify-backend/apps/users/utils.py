...
def recommendation_creator(spotify_recommendations):
    ...
    for track in spotify_recommendations["tracks"]:
        ...
        release_year = track["album"]["release_date"][
            :4
        ]  # Extracting the first 4 characters for the release year
        spotify_id = track["id"]
        ...
        artists_list = ", ".join(
            [artist["name"].title() for artist in track["artists"]]
        )
...
def serializeFriendGroupExtended(friend_group: FriendGroup):
    return {
        ...
        "admin": friend_group.created_by.username,
        "members": [
            {"id": member.id, "username": member.username, "img_url": member.img_url}
