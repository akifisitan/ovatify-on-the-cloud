...
class ArtistModelTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.artist = Artist.objects.create(
            id="artist1", name="Artist 1", bio="Another Artist Bio"
        )
...
class GetAllTemposTest(TestCase):
    ...
    def test_get_all_tempos(self):
        ...
        expected_tempos = [
            {"value": tempo[0], "label": tempo[1]} for tempo in Tempo.choices
        ]
...
class GetBangerSongsTest(TestCase):
    ...
    def test_get_banger_songs_no_match(self):
        response = self.client.get(
            reverse("get_banger_songs"), {"mood": "SAD", "tempo": "FAST"}
        )

        self.assertEqual(response.status_code, 404)
        self.assertIn("message", response.json())
        self.assertEqual(
            response.json()["message"], "No Banger songs found with the given filters"
        )
...
class PlaylistModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.friend_group = FriendGroup.objects.create(
            name="Test Group", created_by=cls.user1
        )
        ...
        cls.playlist1 = Playlist.objects.create(
            name="Playlist 1", user=cls.user1, friend_group=cls.friend_group
        )
        cls.playlist1.songs.add(cls.song1, cls.song2)

        cls.playlist2 = Playlist.objects.create(
            name="Playlist 2", user=cls.user1, friend_group=cls.friend_group
        )
    ...
    def test_get_playlists_of_group(self):
        ...
        )  # Assuming there are 2 playlists in the group
    ...
    def test_create_empty_playlist(self):
        ...
        data = {
            "playlist_name": "My Playlist New",
            "playlist_description": "A new playlist",
            "group_id": self.friend_group.id,
        }
        ...
        response_default_name = self.client.post(
            url, json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response_default_name.status_code, 201)
        default_playlist_name = "My Playlist New 2"
        self.assertIn(
            "Playlist created successfully", response_default_name.json().get("message")
        )
